...
function updateChart(opt, interval, savings, interest, growth, brokerage) {
    ...
    var nvals = [opt, 2*opt];
    var borderColors = ['rgba(228,26,28,0.6)', 'rgba(152,78,163,0.8)'];
    var backgroundColors = ['rgba(228,26,28,0.6)', 'rgba(152,78,163,0.2)'];

    var half = Math.round(opt/2);
    if (half > 1 && half != opt) {
        nvals.unshift(half);
        borderColors.unshift('rgba(77,175,100,0.5)');
        backgroundColors.unshift('rgba(77,175,74,0.5)');
    }
    if (opt > 1) {
        nvals.unshift(1);
        borderColors.unshift('rgba(55,126,184,0.6)');
        backgroundColors.unshift('rgba(55,126,184,0.6)');
    }
